...
class SD6Model_1f1w_ci(mesa.Model):
    ...
    **kwargs : dict
    ...
    """
    def __init__(self, pars, crop_options, aquifers_dict, fields_dict, wells_dict, finances_dict, behaviors_dict, 
        ...
        self.harvest_price_step = kwargs.get("harvest_price_step") # [finance_id][year][crop]
        self.projected_price_step = kwargs.get("projected_price_step") # [finance_id][year][crop]
        self.aph_revenue_based_coef_step = kwargs.get("aph_revenue_based_coef_step") # [finance_id][year][field_type][crop]
    ...
    def step(self):
        ...
        if self.aph_revenue_based_coef_step is not None:
            for unique_id, finance in self.finances.items():
                aph_revenue_based_coef = self.aph_revenue_based_coef_step.get(finance.finance_id)
                if aph_revenue_based_coef is not None:
                    finance.aph_revenue_based_coef = aph_revenue_based_coef[self.current_year]
    ...
    def get_dfs(model):
        ...
        df_agt = pd.concat([df_agt, df_behaviors[['bid', 'Sa', 'E[Sa]', 'Un', 'state', 'profit',
               'revenue', 'energy_cost', 'premium', 'payout', 'gp_status', 'gp_MIPGap']]], axis=1)
