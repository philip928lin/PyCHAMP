...
class Behavior(mesa.Agent):
    ...
    **kwargs
    ...
    def __init__(self, unique_id, model, settings: dict, pars: dict,
                 fields: dict, wells: dict, finance, aquifers: dict, optimization_class: object,
                 **kwargs):
        """
        ...
        self.needs = {}
        self.selected_behavior_id_in_network = None # This will be populated after social comparison
        ...
        if year is None:
            ...
            for fi, field in self.fields.items():
                ...
                for crop, percieved_risk in percieved_risks_f.items():
                    prec_aw_step_f[crop] = prec_aw_step_f[crop]*fotr + percieved_risk*(1-fotr)
            ...
            for fi, field in self.fields.items():
                ...
                perceived_prec_aw_f = {
                    crop: round(percieved_risks_f[crop]*(1-fotr) + prec_aw[crop]*fotr, 4) \
    ...
    def make_dm(self, state, dm_sols, neighbor=None, init=False):
        ...
        for wi, well in wells.items():
            ...
            dm.setup_constr_well(
                well_id=wi, 
                dwl=proj_dwl, 
                st=well.st, l_wt=well.l_wt, r=well.r, k=well.k,
        ...
        dm_sols = dm.sols
        if dm_sols is None:
            warnings.warn("Gurobi returns empty solutions (likely due to infeasible problem.")
    ...
    def make_dm_social_comparison(self):
        ...
        self.selected_behavior_id_in_network = behavior_ids_in_network[select_behavior_index]
    ...
    def make_dm_imitation(self):
        ...
        if selected_behavior_id_in_network is None:
            try:    # if rngen is given in the model
                selected_behavior_id_in_network = self.rngen.choice(self.behavior_ids_in_network)
            except:
                selected_behavior_id_in_network = np.random.choice(self.behavior_ids_in_network)
    ...
    **kwargs
    ...
    def run_simulation(self):
        ...
        for fi, field in fields.items():
            ...
            i_crop = dm_sols[fi]["i_crop"].copy()
            field.step(
                irr_depth=irr_depth, i_crop=i_crop,
                prec_aw=prec_aw_step[field.prec_aw_id][current_year] 
                )
            ...
            if field.premium_dict is not None:
                ...
                for c in self.model.crop_options: 
                    field.premium_dict["rainfed"][c] = finance.cal_APH_revenue_based_premium(
                        df=df, crop=c, county=field.county, field_type="rainfed",
                        projected_price=finance.projected_price[c],
                        aph_yield_dict=field.aph_yield_dict, 
                        premium_ratio=finance.premium_ratio,
                        coverage_level=0.75,
                        field_area=field.field_area)
                    field.premium_dict["irrigated"][c] = finance.cal_APH_revenue_based_premium(
                        df=df, crop=c, county=field.county, field_type="irrigated",
                        projected_price=finance.projected_price[c],
                        aph_yield_dict=field.aph_yield_dict, 
                        premium_ratio=finance.premium_ratio, 
                        coverage_level=0.75, 
    ...
    def make_dm(self, state, dm_sols, neighbor=None, init=False):
        ...
        dm.setup_constr_finance(self.finance.finance_dict, payout_ratio=self.finance.payout_ratio, premium_ratio=self.finance.premium_ratio)
        dm.setup_obj(target=dm_dict["target"], consumat_dict=consumat_dict) # no dynamic update for alpha.
        dm.finish_setup(display_summary=dm_dict['display_summary'])
        dm.solve(
            keep_gp_model=dm_dict['keep_gp_model'],
            keep_gp_output=dm_dict['keep_gp_output'],
