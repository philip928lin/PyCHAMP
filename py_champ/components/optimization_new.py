...
class Optimization():
    ...
    """
    def __init__(self, unique_id="", gpenv=None):
        """
    ...
    def setup_constr_field(self, field_id, field_area, prec_aw, 
        ...
        **kwargs: 
        ...
        a_te  = m.addVar(vtype="C", name=f"{fid}.a_te", lb=-inf, ub=inf)
        b_te  = m.addVar(vtype="C", name=f"{fid}.b_te", lb=-inf, ub=inf)
        ...
        m.addConstr(gp.quicksum(i_te[i] for i in range(n_te)) == 1,
                    name=f"c.{fid}.i_te")
        m.addConstr(a_te == gp.quicksum( techs[te][0] * i_te[i] \
                    for i, te in enumerate(tech_options) ),
                    name=f"c.{fid}.a_te")
        m.addConstr(b_te == gp.quicksum( techs[te][1] * i_te[i] \
                    for i, te in enumerate(tech_options) ),
                    name=f"c.{fid}.b_te")
        ...
        self.vars[fid]['a_te'] = a_te
        self.vars[fid]['b_te'] = b_te
    ...
    def setup_constr_well(self, well_id, dwl, B, l_wt,
                          eff_pump,
        ...
        a_te  = m.addVar(vtype="C", name=f"{wid}.a_te", lb=-inf, ub=inf)
        b_te  = m.addVar(vtype="C", name=f"{wid}.b_te", lb=-inf, ub=inf)
        ...
        self.vars[wid]['a_te'] = a_te
        self.vars[wid]['b_te'] = b_te
        self.n_wells += 1
    ...
    def finish_setup(self, display_summary=True):
        ...
        for k, wid in enumerate(wids):
            ...
            m.addConstr((vars[wid]['a_te'] == gp.quicksum(vars[fid]['a_te'] * allo_r[f,k,:] \
                        for f, fid in enumerate(fids))), name=f"c.{wid}.a_te")
            m.addConstr((vars[wid]['b_te'] == gp.quicksum(vars[fid]['b_te'] * allo_r[f,k,:] \
                        for f, fid in enumerate(fids))), name=f"c.{wid}.b_te")
