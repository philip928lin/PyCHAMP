...
class Optimization:
    ...
    def setup_constr_field(
        ...
        **kwargs:
        ...
        m.addConstr(
            a_te
            == gp.quicksum(techs[te][0] * i_te[i] for i, te in enumerate(tech_options)),
            name=f"c.{fid}.a_te",
        )
        m.addConstr(
            b_te
            == gp.quicksum(techs[te][1] * i_te[i] for i, te in enumerate(tech_options)),
            name=f"c.{fid}.b_te",
        )
        m.addConstr(
            l_pr
