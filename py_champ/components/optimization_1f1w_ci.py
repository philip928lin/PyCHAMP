...
class Optimization_1f1w_ci():
    ...
    def setup_ini_model(self, unique_id, gpenv, horizon=1, 
    ...
    def setup_constr_field(self, field_id, field_area, prec_aw, 
                           water_yield_curves, field_type="optimize", 
                           i_crop=None, i_rainfed=None,
                           premium_dict=None, aph_yield_dict=None, **kwargs):
        ## Append field_id
        ...
        if i_rainfed is not None:
            if np.sum(i_rainfed) > 0.5: # Avoid numerical issue (should be 1 if rainfed) 
                field_type = "rainfed"
            else:
        ...
        if field_type == "rainfed":
            ...
            if i_rainfed_input is not None:
                m.addConstr(i_rainfed == i_rainfed_input,
                            name=f"c.{fid}.i_rainfed_input")
            ...
            m.addConstr(i_crop - i_rainfed >= 0,
                        name=f"c.{fid}.i_rainfed")
            m.addConstr(irr_depth == 0, name=f"c.{fid}.irr_rain_fed")
            
        elif field_type == "irrigated":
            m.addConstr(i_rainfed == 0,
                        name=f"c.{fid}.no_i_rainfed")
        ...
        m.addConstrs((w_[ci,hi] == gp.min_(w_temp[ci,hi], constant=1) \
                    for ci in range(n_c) for hi in range(n_h)),
        ...
        m.addConstr(v == gp.quicksum(v_c[j,:] for j in range(n_c)),
                    name=f"c.{fid}.v(m-ha)")
        m.addConstr(y_y == gp.quicksum( y_[j,:] for j in range(n_c) ),
                    name=f"c.{fid}.y_y")
    ...
    def setup_constr_well(self, well_id, dwl, B, l_wt, eff_pump,
                          pumping_capacity=None, rho=1000., g=9.8016):
    ...
    def setup_constr_finance(self, finance_dict, payout_ratio=1, premium_ratio=1):
        ...
        harvest_price = {
            c: finance_dict['harvest_price'][c] \
                for c in crop_options
            }
        
        projected_price = {
            c: finance_dict['projected_price'][c] \
                for c in crop_options
            }
        ...
        m.addConstr(rev == gp.quicksum(y[j,:] * crop_profit[c] \
        ...
        if premium_dict is not None:
            for j, c in enumerate(crop_options):
                for r in range(n_c):
                    ...
                    m.addConstr(premium == i_cr * premium_dict["rainfed"][c], name=f"c.premium_{c}_{r}_rainfed")
                    ...
                    m.addConstr(i_cr >= i_crop[j] + i_rainfed[r] - 1)
                    m.addConstr(premium == i_cr * premium_dict["irrigated"][c] * premium_ratio, name=f"c.premium_{c}_{r}_irrigated")
        else:
        ...
        if aph_yield_dict is not None:
            ...
            for j, c in enumerate(crop_options):
                ...
                payout_c = m.addMVar((n_h), vtype="C", name=f"payout_{c}(1e4$)", lb=0, ub=inf)
                
                payout_temp = m.addMVar((n_h), vtype="C", name="payout_temp", lb=-inf, ub=inf)
                m.addConstrs((payout_temp[hi] == h_or_p * 0.75 * aph_yield_dict["irrigated"][c] * (1-i_rainfed[j, 0]) + h_or_p * 0.75 * aph_yield_dict["rainfed"][c] * i_rainfed[j, 0] - harvest_price[c] * y[j, hi]) for hi in range(n_h))
                
                
                m.addConstrs((payout_c[hi] == gp.max_(payout_temp[hi], constant=0) \
                             for hi in range(n_h)),
                            name=f"c.payout_c_{c}") # w_ = minimum of 1 or w/w_max
                    

                payout_crops.append(payout_c)
            
            for hi in range(n_h):
                m.addConstr(payout[hi] == gp.quicksum(payout_c[j] * i_crop[j] \
                            for j, c in enumerate(crop_options)) * payout_ratio,
                            name="c.payout")       
        else:
    ...
    def setup_constr_wr(self, water_right_id, wr_depth,
        ...
        if remaining_tw is not None and remaining_wr is not None:
            m.addConstr(
                gp.quicksum(irr_sub[j,h] \
                for j in range(n_c) \
                for h in range(remaining_tw)) <= remaining_wr,
        ...
        while remaining_length >= time_window:
            m.addConstr(
                gp.quicksum(irr_sub[j,h] \
                for j in range(n_c) \
                for h in range(start_index, start_index+time_window)) <= wr_depth,
                name=f"c.{water_right_id}.wr_{c_i}(cm)"
                )
            c_i += 1
        ...
        if remaining_length > 0:
            ...
            m.addConstr(
                gp.quicksum(irr_sub[j,h] \
                for j in range(n_c) \
                for h in range(start_index, n_h)) <= wr_tail,
                name=f"c.{water_right_id}.wr_{c_i}(cm)"
                )
    ...
    def setup_obj(self, target="profit", consumat_dict={"alpha": {"profit": 1}, "scale": {"profit": 0.23 * 50}}):
        """
    ...
    def finish_setup(self, display_summary=True):
        ...
        payout = vars['payout']
        m.addConstr((profit == (payout + rev - cost_e - annual_cost - premium)/n_f), name="c.profit")
